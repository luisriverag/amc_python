...
`verify_fixtures` exit checks below. Both redistribution restrictions this
status previously named are resolved: `src/original/ElTree/`, the RAR archive
that also contained it, and the unlicensed `Common/ComboBoxAutoWidth.pas`
have all been removed from the repository and its git history, since ElTree's
license permits distribution only inside compiled software,
`ComboBoxAutoWidth.pas` had no license grant at all, and AMC Python never used
either. Sprint 1's only remaining required work — reacquiring the published
source archive from an independently recorded origin — still cannot be
performed inside this repository's automated development environment and
remains externally blocked. Sprints 2–4 stay gated behind Sprint 1's exit
checks exactly as
...
1. **Compatibility evidence before more inferred format behavior.** Native and XML
   ...
   fixture.
2. **Redistribution clearance before release work.** The ElTree and
   `Common/ComboBoxAutoWidth.pas` restrictions are both resolved (by removing
   the affected files, not by obtained permission). Historical source must
   still not be included in a release before `antcomponents.zip`'s publisher
   authentication and the remaining `Common`/`antcomponents` per-file review
   are complete.
...
- Reacquire the published source archive from an independently recorded origin;
  ...
  against the remaining 847-file expanded inventory (`src/original/` minus
  ElTree and `ComboBoxAutoWidth.pas`, plus `src/antcomponents/`) using
  `tools/acquire_upstream.py`, but would not itself be re-committed. RAR
  acquisition tries each installed extractor in order, so one incompatible
  tool does not prevent a later capable extractor from completing the
  evidence check.
- Compare a reacquired `antcomponents.zip` with the checked-in copy and its
  76-file expanded inventory the same way.
- [Done] Both the ElTree source-redistribution restriction and the absent
  license grant for `Common/ComboBoxAutoWidth.pas` are resolved: rather than
  obtaining permission or maintaining a documented non-distributed evidence
  workflow, `src/original/ElTree/`, the RAR archive that also contained it,
  and `ComboBoxAutoWidth.pas` itself were removed entirely from the
  repository and its git history, since AMC Python never used either.
