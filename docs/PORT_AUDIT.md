...
| Picture prototype | Linked and byte/pixel-bounded validated embedded set/clear/crop, atomic batch clearing across an extended selection, JSON/native retention, and atomic export | Service/CLI/GUI/native tests, including malformed image, crop/bounds, batch-clear atomicity, and failure preservation | Moderate internally; upstream path/conversion semantics unverified |
...
| Pictures | `TMoviePicture` in `movieclass.pas`, picture forms | Link/embed/clear/export/crop, bounded poster display, and atomic batch clearing | Upstream import modes, naming/copy/move rules, conversion options, batch embed/link import, and genuine embedded/linked fixtures |
