...
| `media.py` | Bounded media discovery and dependency-free file/WAV/FLAC/AIFF facts | Network access or UI behavior |
...
- `media.py` provides portable filesystem facts and PCM WAV/FLAC/AIFF
  metadata, parsed directly from each format's mandatory header using only
  the standard library (AIFF deliberately reimplements its 80-bit
  extended-float sample rate rather than using the deprecated, Python-3.13-
  removed `aifc` module). Full codec inspection for compressed/lossy formats
  belongs behind a
