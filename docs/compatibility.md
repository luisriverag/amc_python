...
| Pictures | investigating | Service/CLI can link, byte/pixel-bound validate/embed, crop, clear, and atomically export movie pictures, including atomic batch clearing across distinct movie numbers from the CLI (`picture-clear NUMBER [NUMBER ...]`) and the desktop **Clear Pictures** toolbar action; native read/write retains embedded bytes in JSON; synthetic service/CLI/GUI/native tests | Verify external-path resolution, crop/conversion behavior, and embedded/link semantics with genuine upstream fixtures; batch embedding/linking still requires one source image per movie |
