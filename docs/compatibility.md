...
| Media analysis | prototype | Portable file facts and dependency-free PCM WAV, FLAC, AIFF/AIFF-C, and MP3 duration/audio data can create movie entries; MP3 duration/bitrate is computed from the first MPEG audio frame header and file size (exact for CBR files, approximate for VBR files without a parsed Xing/VBRI header — documented limitation, not upstream-verified) | Add MP4 and OGG dependency-free parsing (or a bounded optional codec provider) and map upstream field/filter semantics |
