...
| Native `.amc` read | investigating | User-designated source baseline drives fixed-record 1.0–3.0 and modern 3.1–4.2 parsing, bounded collections, truncation checks, native→JSON retention, and preservation of undefined CP-1252 bytes; no genuine fixtures | Generate genuine fixtures, determine real catalog code pages, cross-check every version, and broaden fuzz/property coverage |
...
| Native `.amc` write | investigating | Explicit, atomic AMC 4.2 export with synthetic byte round trips for metadata, custom values, pictures, and supplementary records; strict retained-rating, custom-field, separator, and structural metadata validation; configurable budgets exposed by CLI | Cross-check files in upstream 4.2.3.2 before compatibility claims |
...
| Pictures | investigating | Service/CLI can link, byte/pixel-bound validate/embed, crop, clear, and atomically export movie pictures; native read/write retains embedded bytes in JSON; synthetic service/CLI/native tests | Verify external-path resolution, crop/conversion behavior, and embedded/link semantics with genuine upstream fixtures |
...
| CLI CRUD | partial | In-process function tests, explicit import policies, shared application services for failure-atomic mutations, exports, and validated backup/restore, plus an installed empty-list JSON contract | Expand installed end-to-end command behavior contracts |
| Desktop GUI | prototype | File workflows, responsive multi-row controls, safe HTTP(S) movie URL launching, atomic multi-row removal and checked-state updates, bounded failure-atomic undo/redo, bidirectional column sorting, validated scalar and multiline description/comment editing, checked/loan filters, table/details/poster layouts, scaled Pillow decoding plus linked/embedded poster editing, statistics, duplicates, shortcuts, and catalog-path entry point have headless tests | Add batch picture management, cropping, history preferences, and widget/display tests |
