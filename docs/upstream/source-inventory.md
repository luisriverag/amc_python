...
| `Common/MediaInfo.pas`, `Movie Catalog/getmedia.pas` | Media metadata extraction | `amc.media` | prototype, not compared | synthetic file/WAV/FLAC/AIFF/MP3 tests | Portable path/name/extension/size and WAV/FLAC/AIFF/MP3 audio facts only, parsed directly from each format's own public specification rather than calling `MediaInfo.dll` the way upstream does for every format including WAV; MP3 duration/bitrate come from the first MPEG audio frame header and are exact for CBR files, approximate for VBR files without a parsed Xing/VBRI header; MP4 and OGG are unimplemented; upstream exposes 28 media tags and filtering/merge behavior |
