 ...
 *     by antp : disabled Corel style if XP theme available
 * ----------------------------------------------------------------------------
 *)
unit AntCorelButton;
...
uses
  ...
  {$IFDEF ANTUNICODE}
  TntStdCtrls, TntWindows,
  {$ENDIF}
  StdCtrls;

type
  TCorelButton = class({$IFDEF ANTUNICODE}TTntButton{$ELSE}TButton{$ENDIF})
  private
    FCanvas: TCanvas;
    IsFocused: Boolean;
...
//      Flags := Flags or DFCS_INACTIVE;
    ...
    with FCanvas do begin
      ...
      if Enabled or ((DrawItemStruct.itemState and ODS_DISABLED) = 0) then begin
        {$IFDEF ANTUNICODE}
        Tnt_DrawTextW(Handle, PWideChar(Caption), Length(Caption), R, DT_CENTER or
