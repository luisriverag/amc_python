...
{                                                       }
{       21-10-2009 AS: CMYK Decoding added              }
{                                                       }
{*******************************************************}
...
begin
  Bitmap.Canvas.Lock; {Fix}
  try
    ACanvas.StretchDraw(Rect, Bitmap);
  finally
    Bitmap.Canvas.Unlock; {Fix}
  end;
end;
...
procedure DoConvertCMYK2RGB(Bmp: TBitmap);
var
  I, J       : Integer;
  C, M, Y, K : Integer;
  R, G, B    : Integer;
...
begin
  ...
  try
    try
      ...
      else if jc.d.out_color_space in [JCS_CMYK, JCS_YCCK] then // 21-10-2009 AS
...
{$ENDIF}
              if jc.d.out_color_space in [JCS_CMYK,JCS_YCCK] then // 21-10-2009 AS
              Begin
                PPixel := Pointer(DestScanLine);

                for nI := 0 to FBitmap.Width -1 do
                Begin
                  // CMYK to RGB conversie
                  C := PPixel^ and $FF;
                  M := (PPixel^ shr 8) and $FF;
                  Y := (PPixel^ shr 16) and $FF;
                  K := PPixel^ shr 24;

                  R := (Y * K) div 255;
                  G := (M * K) div 255;
                  B := (C * K) div 255;
                  PPixel^ := R or (G shl 8) or (B shl 16);
                  Inc(PPixel);
                end;
              end
              else if jc.d.out_color_space = JCS_YCbCr then
              Begin
                CurPixel := DestScanLine;
