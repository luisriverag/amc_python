...
{ Added by soulsnake
  Same as TBCustomLoadPositions/TBCustomSavePositions but save all toolbars in
  all components and sub-components.
  Constrains: All toolbars and docks must have different names in
  all components and sub-components to be saved and lodead correcly.}
procedure TBCustomLoadPositionsAllComponents(const AComponent: TComponent;
  const ReadIntProc: TTBPositionReadIntProc;
  const ReadStringProc: TTBPositionReadStringProc;
  const ExtraData: TTBPositionExtraData);
procedure TBCustomSavePositionsAllComponents(const AComponent: TComponent;
  const WriteIntProc: TTBPositionWriteIntProc;
  const WriteStringProc: TTBPositionWriteStringProc;
  const ExtraData: TTBPositionExtraData);
  ...
  for I := 0 to OwnerComponent.ComponentCount-1 do
    if OwnerComponent.Components[I] is TTBCustomDockableWindow then
      with TTBCustomDockableWindow(OwnerComponent.Components[I]) do begin
        if TTBCustomDockableWindow(OwnerComponent.Components[I]).Name = '' then
          Continue;
        if Floating then
...
var
  ...
  procedure DisableDocks(PComponent: TComponent);
  var
    I: Integer;
  begin
    with PComponent do
      for I := 0 to ComponentCount-1 do
        ...
        else if Components[I].ComponentCount > 0 then begin
          DisableDocks(Components[I]);
        end;
  end;

  function FindDock(AName: String; PComponent: TComponent): TTBDock;
  var
    I: Integer;
  begin
    Result := nil;
    for I := 0 to PComponent.ComponentCount-1 do
      if (PComponent.Components[I] is TTBDock) and
         {$IFNDEF CLR}
         (CompareText(PComponent.Components[I].Name, AName) = 0) then begin
         {$ELSE}
         SameText(PComponent.Components[I].Name, AName, loInvariantLocale) then begin
         {$ENDIF}
        Result := TTBDock(PComponent.Components[I]);
        Break;
      end
      else if PComponent.Components[I].ComponentCount > 0 then begin
        Result := FindDock(AName, PComponent.Components[I]);
        if Result <> nil then
          Break;
      end;
  end;

  procedure ReadValues(const Toolbar: TTBCustomDockableWindow;
    const NewDock: TTBDock; Rev: Integer);
  var
    Pos: TPoint;
    Data: TTBReadPositionData;
    LastDockName: String;
    ...
    with Toolbar do begin
      ...
      if Assigned(NewDock) then
        ...
        if (Rev >= 3) and FUseLastDock then begin
          LastDockName := ReadStringProc(Name, rvLastDock, '', ExtraData);
          if LastDockName <> '' then begin
            ADock := FindDock(LastDockName, AComponent);
            if Assigned(ADock) then
              LastDock := ADock;
          end;
        end;
  ...
  procedure LoadTB(PComponent: TComponent);
  var
    Rev: Integer;
    I: Integer;
    ToolWindow: TComponent;
  ...
  begin
    for I := 0 to PComponent.ComponentCount-1 do begin
      ToolWindow := PComponent.Components[I];
      if ToolWindow is TTBCustomDockableWindow then
        with TTBCustomDockableWindow(ToolWindow) do begin
          Toolbar := TTBCustomDockableWindow(ToolWindow);
          {}{should skip over toolbars that are neither Docked nor Floating }
          ...
          if Rev = 2000 then begin
            ...
            if DockedToName <> '' then begin
              if DockedToName <> rdDockedToFloating then begin
                ADock := FindDock(DockedToName, AComponent);
                if (ADock <> nil) and (ADock.FAllowDrag) then
                  ReadValues(TTBCustomDockableWindow(ToolWindow), ADock, Rev);
              end
              else
                ReadValues(TTBCustomDockableWindow(ToolWindow), nil, Rev);
            end;
          end;
        end
      else if ToolWindow.ComponentCount > 0 then
        LoadTB(ToolWindow);
    end;
  end;
...
procedure TBCustomSavePositionsAllComponents(const AComponent: TComponent;
  ...
  procedure SaveTB(const PComponent: TComponent);
  var
    I: Integer;
    N, L: String;
    Data: TTBWritePositionData;
  begin
    for I := 0 to PComponent.ComponentCount-1 do
      if (PComponent.Components[I] is TTBCustomDockableWindow) then
        with TTBCustomDockableWindow(PComponent.Components[I]) do begin
          if Name = '' then
            Continue;
          if Floating then
            N := rdDockedToFloating
      ...
      else if PComponent.Components[I].ComponentCount > 0 then
        SaveTB(PComponent.Components[I]);
  end;
