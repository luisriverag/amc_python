...
def test_check_tool_runs_documented_checks_in_order(monkeypatch):
    ...
    assert calls[5][0][1:] == ["tools/check_license_inventory.py"]
    assert calls[6][0][1:] == ["tools/verify_fixtures.py"]
    assert calls[7][0][1:] == ["-m", "amc.cli", "--help"]
    assert calls[7][1]["PYTHONPATH"].split(MODULE.os.pathsep)[0] == str(MODULE.ROOT / "src")
    assert calls[8][0] == ["git", "diff", "--check"]
    assert calls[9][0] == ["git", "diff", "--cached", "--check"]
