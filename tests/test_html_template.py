...
def test_render_full_template_expands_item_loop_and_general_tags():
    ...
    assert page == (
        "<html>Ripley has 2 movies:"
        "<p>1: Alien (1979)</p><p>2: Aliens (1986)</p>"
        "</html>"
    )
...
def test_render_individual_template_covers_common_movie_tags():
    catalog = Catalog()
    movie = Movie(
        number=7, title="Moon", original_title="Moon", year=2009, director="Duncan Jones",
        rating=8.4, user_rating=9.0, checked=True, actors="Sam Rockwell",
        description="Solitude.\nIsolation.", picture="posters/moon.jpg",
    )
    template = (
...
def test_render_individual_template_appreciation_bucket_matches_upstream_ranges():
    ...
    catalog = Catalog()
    cases = [
        (2.9, "0"), (3.0, "1"), (4.9, "1"), (5.0, "2"), (6.9, "2"),
        (7.0, "3"), (8.9, "3"), (9.0, "4"), (10.0, "4"),
    ]
...
def test_export_html_template_writes_full_and_individual_pages_atomically(tmp_path: Path):
    ...
    written = export_html_template(
        catalog, destination, full_template=full, individual_template=individual,
    )
    ...
    assert set(written) == {
        destination, individual_dir / "1.html", individual_dir / "5.html",
    }
...
def test_export_html_template_uses_a_custom_individual_filename_pattern(tmp_path: Path):
    ...
    written = export_html_template(
        catalog, tmp_path / "out.html",
        individual_template=individual, individual_dir=tmp_path / "pages",
...
def test_export_html_template_rejects_an_oversized_template(tmp_path: Path):
    ...
    with pytest.raises(ValueError, match="exceeds size limit"):
        export_html_template(catalog, tmp_path / "out.html", full_template=huge, max_template_bytes=1)
...
def test_cli_export_html_template_writes_full_page(tmp_path: Path, capsys):
    ...
    template.write_text("$$ITEM_BEGIN$$ITEM_FORMATTEDTITLE ($$ITEM_YEAR)$$ITEM_END", encoding="utf-8")
    destination = tmp_path / "out" / "index.html"

    assert main([
        "-c", str(catalog_path), "export-html-template", str(destination),
        "--full-template", str(template),
    ]) == 0
